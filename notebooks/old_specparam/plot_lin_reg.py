...
ch_effects
#%%
(ch_effects['hdi_3%'] > 0.1).sum()

#%%
#%%
(ch_effects['hdi_97%'] < -0.1).sum()


#%%
stc_parc = (np.concatenate([[ch_effects['mean'][0]], ch_effects['mean']]))
stc_mask = (np.concatenate([[ch_effects['hdi_97%'][0]], ch_effects['hdi_97%']])) > 0
...
def plot_parc(stc_parc, stc_mask, cmap, parc='HCPMMP1'):
    ...
    for hemi in ["lh", "rh"]:
        ...
        brain.add_data(vtx_data, hemi=hemi, fmin=stc_parc.min(), fmid=stc_parc.mean(),
                       fmax=stc_parc.max(), 
...
#%%
eff_brain = plot_parc(stc_parc, stc_mask, 'Blues')
