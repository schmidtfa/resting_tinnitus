...
# %%
distress = df_matched.query('tinnitus == True')['tinnitus_distress']
# %%
distress_labels = pd.cut(distress, [-0.1, 7, 12, 18, 24,], labels=['compensated', 'moderate distress', 'severely distressed', 'most severely distressed']).astype('str')
distress_labels[distress_labels == 'nan'] = 'not measured'
# %%

labels, counts = np.unique(distress_labels, return_counts=True)

pd.DataFrame(dict(zip(labels, counts)), index=[0])


# %%
df_matched['measurement_age'].min()

# %%
df_matched['measurement_age'].max()

# %%
df_matched['measurement_age'].mean()

# %%
df_matched['measurement_age'].std()
