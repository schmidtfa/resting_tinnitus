...
def data_loader(path2data, freqs, heart_thresh, eye_thresh, peaks, fit_knee, sss, interpolate=False, get_psd=False):
    ...
    for idx, file in enumerate(all_files):
        ...
        if 'ecg_scores' in cur_data.keys() and (cur_data['ecg_scores'] > heart_thresh).sum() > 0:
            ...
            #heart_shape = cur_data['data_heart']['mag']['f_by_f_error'].mean(axis=0).shape
            #no_ica_shape = cur_data['data_no_ica']['mag']['f_by_f_error'].mean(axis=0).shape
            #ica_shape = cur_data['data_brain']['mag']['f_by_f_error'].mean(axis=0).shape

            #print(f'heart shape is {heart_shape}')
            #print(f'no ica shape is {no_ica_shape}')
            #print(f'ica shape is {ica_shape}')
            #print(f'fres shape is {fooof_freqs.shape}')

            all_f_by_f.append(pd.DataFrame({'heart': np.median(cur_data['data_heart']['mag']['f_by_f_error'], axis=0),
                                                'no_ica': np.median(cur_data['data_no_ica']['mag']['f_by_f_error'], axis=0),
                                                'ica': np.median(cur_data['data_brain']['mag']['f_by_f_error'], axis=0),
                                                'Frequency (Hz)': fooof_freqs,
                                                'subject_id': cur_data['subject_id'],
                                                }))
    ...
    if get_psd:
        ...
        return df_cmb, df_cmb_psd, all_f_by_f

    else:
        df_cmb = pd.concat(all_df)
        df_f_by_f = pd.concat(all_f_by_f).melt(id_vars=['subject_id', 'Frequency (Hz)'])
        return df_cmb, df_f_by_f