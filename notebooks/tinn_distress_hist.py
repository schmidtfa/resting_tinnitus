...
data_dir = join(home_base, 'data/lin_reg/')
# %%
...
for key, val in feature_dict.items():
    ...
    if val == None:
        ...
        summary = az.summary(mdf, var_names='psi_beta|', hdi_prob=.89)
        summary['feature'] = key
...
# %%
thresh = 0.18
eff_df_cmb['positive effect'] = eff_df_cmb['hdi_5.5%'] > thresh
eff_df_cmb['negative effect'] = eff_df_cmb['hdi_94.5%'] < -thresh
eff_df_cmb['null effect'] = np.logical_and(eff_df_cmb['hdi_5.5%'] > -thresh, eff_df_cmb['hdi_94.5%'] < thresh)
#i might need some additional specifications for below
eff_df_cmb['leaning positive effect'] = np.logical_and(eff_df_cmb['mean'] > thresh, np.logical_and(eff_df_cmb['hdi_5.5%'] > 0., eff_df_cmb['hdi_5.5%'] < thresh))
eff_df_cmb['leaning negative effect'] = np.logical_and(eff_df_cmb['mean'] < -thresh, np.logical_and(eff_df_cmb['hdi_94.5%'] < 0., eff_df_cmb['hdi_94.5%'] > -thresh))
...
f.savefig('../results/hist_distress_lin_reg_effects_hurdle.svg')
...
#6 <- normally 10 for hurdle
ch_effects['ch_name'] = [ch[10:-1] for ch in ch_effects['ch_name']]
...
fig.savefig(f'../results/brain_lin_{cur_param}_tinnitus_distress_lateral_hurdle.svg')

# %%
ch_effects[ch_effects['Effect'] > 0.10]
# %%
ch_effects['Effect'].max()
# %%
