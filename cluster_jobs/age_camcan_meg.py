...
class AgeReg(Job):
    ...
    def run(self,
            ...
            #feature = 'Exponent_2'
            df_ap = pd.read_csv('/home/schmidtfa/experiments/brain_age/data/data_cam_can/aperiodic_params.csv')
            df_pe = pd.read_csv('/home/schmidtfa/experiments/brain_age/data/data_cam_can/periodic_params.csv')

            cur_df_f = df_ap.merge(df_pe, on=['ch_name', 'subject_id', 'age'])
    ...
    def _run_hierarchical_log_reg(self, 
        ...
        #feature = 'tau'
        cur_df = df[['cortex_info', 'ch_name', feature, 'subject_id', 'age']].dropna()



        if feature in ['Exponent_1', 'tau']:
             if feature == 'tau': 
                  cur_df[feature] += 1e-100 #some weird subjects have tscale of 0
        ...
        cur_df[feature] = (
               cur_df.groupby("ch_name")[feature]
                      .transform(lambda x: (x - np.nanmean(x)) / (np.nanstd(x)))
)
        ...
        with pm.Model(coords=coords) as glm:
            ...
            beta_m2 = pm.Normal('beta_m2', mu=0, sigma=.5, dims='ch_name')
        ...
        ch_effects.to_csv(f'/home/schmidtfa/experiments/resting_tinnitus/data/age_cc_final_final/{feature}_hi.csv')
        mdf.to_netcdf(f'/home/schmidtfa/experiments/resting_tinnitus/data/age_cc_final_final/{feature}_hi.nc')
