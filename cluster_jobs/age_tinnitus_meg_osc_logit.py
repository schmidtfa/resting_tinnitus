...
class AgeReg(Job):
    ...
    def run(self,
            ...
            #%%
            self._run_hierarchical_log_reg(df_cmb, 
                                           feature, 
                                           df_regions_info, 
                                           sample_kwargs)
            
                # %% define regression model
    def _run_hierarchical_log_reg(self, 
                                  df, 
                                  feature, 
                                  df_regions_info, 
                                  sample_kwargs):
        #%%
        #feature = 'alpha_cf'
        cur_df = df[['cortex_info', 'ch_name', feature, 'tinnitus', 'subject_id', 'age']]
        ...
        cur_df['age_z'] = standardize(cur_df['age'])
        
        #effect coding for tinnitus to reduce multicollinearity
        cur_df['tinnitus'] = cur_df['tinnitus'] - .5

     #    #4debug
     #    if 'alpha_pw' in feature:
     #         cur_df[feature][np.isnan(cur_df[feature]) == True] = 0
     #    elif 'alpha_cf' in feature:
     #         cur_df = cur_df[np.isnan(cur_df[feature]) == False].copy() 
     #    elif 'alpha_bw' in feature:
     #         cur_df = cur_df[np.isnan(cur_df[feature]) == False].copy() 

     #    #cur_df[feature] = standardize(cur_df[feature])
     ...
     #    coords['effect'] = ["age", "tinnitus", "age:tinnitus"]
        cur_df = cur_df.merge(df_regions_info[['ch_name', 'cortex_info']], on='ch_name').reset_index()

        channels = cur_df['ch_name'].unique()  # or alpha_df
        n_ch = len(channels)
        ch_cat = pd.Categorical(cur_df['ch_name'], categories=channels)
        ch_ixs = ch_cat.codes   # length = n_obs

        coords = {
          "ch_name": channels,
          "obs_id": np.arange(len(cur_df)),
          }
        
        df_ch2cort = (
        df_regions_info[['ch_name', 'cortex_info']]
          .drop_duplicates()
          .set_index('ch_name')
          )

        # align to the channel order used in coords["ch_name"]
        cortex_for_channels = df_ch2cort.loc[channels, 'cortex_info']

        ch2cort_ixs, cortices = pd.factorize(cortex_for_channels)
        coords["cortices"] = cortices
        coords["effect"] = ["age", "tinnitus", "age:tinnitus"]
        ...
        with pm.Model(coords=coords) as glm:
            ...
            y = pm.Bernoulli('y',
                            p=pm.invlogit(alpha_ch[ch_ix] + pt.batched_dot(beta_ch[ch_ix], X)),
                            observed=y_s,
                            #nu=nu,
                            #sigma=sigma,
                            dims='obs_id'
        ...
        ch_effects.to_csv(f'/home/schmidtfa/experiments/resting_tinnitus/data/age_tinn_final_rkf/{feature}_hi.csv')
        mdf.to_netcdf(f'/home/schmidtfa/experiments/resting_tinnitus/data/age_tinn_final_rkf/{feature}_hi.nc')
